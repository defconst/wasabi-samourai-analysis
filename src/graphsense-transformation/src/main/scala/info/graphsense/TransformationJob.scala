...
object TransformationJob {
  ...
  def main(args: Array[String]) {
    ...
    val mixingTransactions = spark.read
      .format("csv")
      .option("header", true)
      .load(f"$hdfsPath/coinjoin_tx_hash.csv")
      .select(hexStringToByteArray($"tx_hash").as("txHash"))
      .persist()
    ...
    val blocks =
      cassandra
        .load[Block](conf.rawKeyspace(), "block")
        .filter(col(F.blockId) < 725349)
    val transactionsNormal =
      cassandra
        .load[Transaction](conf.rawKeyspace(), "transaction")
        .join(mixingTransactions, Seq(F.txHash), "left_anti")
    val transactionsMixing =
      cassandra
        .load[Transaction](conf.rawKeyspace(), "transaction")
        .join(mixingTransactions, Seq(F.txHash), "right")
        .withColumn(F.coinjoin, lit(false))
    val transactions = transactionsNormal
      .union(transactionsMixing)
      .filter(col(F.blockId) < 725349)
      .as[Transaction]
      .persist()
